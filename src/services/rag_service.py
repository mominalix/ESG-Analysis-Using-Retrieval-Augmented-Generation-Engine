...
class AdvancedRAGService(LoggingMixin):
    ...
    def __init__(self):
        self.esg_system_prompt = """You are an expert ESG (Environmental, Social, Governance) analyst with deep knowledge of corporate sustainability reporting frameworks including CSRD, GRI, SASB, TCFD, and EU Taxonomy.
...
- Highlight any potential compliance gaps or risks
...
Context will be provided from relevant documents. Use only this context to answer questions."""
        ...
        self.query_decomposition_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert at breaking down complex ESG-related questions into simpler sub-questions.
...
Focus on ESG frameworks, compliance, reporting standards, and sustainability practices."""),
    ...
    async def query(
        ...
        **kwargs: Any
        ...
        try:
            ...
            raise ESGAnalysisException(f"RAG query failed: {str(e)}")
        finally:
    ...
    async def _simple_query(
        ...
        **kwargs: Any
        ...
        llm_response = await self._generate_response(
            question, context_documents, esg_framework
        )
    ...
    async def _rerank_results(
        ...
        # Simple re-ranking based on content length and ESG keywords
        esg_keywords = [
            "sustainability", "ESG", "environmental", "social", "governance",
            "CSRD", "GRI", "SASB", "TCFD", "carbon", "emissions", "compliance"
        ]
        ...
        def calculate_relevance_score(result: SearchResult) -> float:
            ...
            # Boost for ESG keywords
            esg_boost = sum(0.1 for keyword in esg_keywords if keyword.lower() in content)
            
            # Boost for query term presence
            query_terms = query_lower.split()
            query_boost = sum(0.2 for term in query_terms if term in content)
            
            # Penalty for very short content
            length_penalty = 0 if len(content) < 100 else 0.1
            
            return base_score + esg_boost + query_boost + length_penalty
...
Please provide a comprehensive answer based on the context documents. Include specific citations (e.g., [Source 1]) for all claims and recommendations."""
    ...
    async def _process_parallel_queries(
        ...
        tasks = [
            self._simple_query(sub_query, esg_framework, search_strategy, k // len(decomposition.sub_queries))
    ...
    async def _process_sequential_queries(
        ...
        for sub_query in decomposition.sub_queries:
            ...
            response = await self._simple_query(
                enhanced_query, esg_framework, search_strategy, k // len(decomposition.sub_queries)
            )
    ...
    async def _synthesize_responses(
        ...
        sub_responses: List[RAGResponse],
        esg_framework: Optional[str]
    ) -> RAGResponse:
...
Please synthesize these into a single, well-structured response that addresses the original question comprehensively. Maintain all citations and ensure logical flow."""
    ...
    def _calculate_confidence(
        ...
        # Boost confidence if response contains citations
        citation_boost = 0.1 if "[Source" in llm_response.content else 0.0
        
        # Boost confidence based on response length (more detailed = higher confidence)
        length_boost = min(0.2, len(llm_response.content) / 2000)
        
        confidence = min(1.0, avg_search_score + citation_boost + length_boost)
