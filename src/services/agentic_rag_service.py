...
from .rag_service import rag_service, RAGResponse
...
class PlannerAgent(LoggingMixin):
    ...
    def __init__(self):
        self.planning_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert ESG report planning agent. Your role is to analyze the available data and requirements to create a comprehensive, structured plan for report generation.
...
Be adaptive - the plan should match the available data and be realistic about what can be accomplished."""),
    ...
    async def create_plan(self, report_type: str, framework: Optional[str], 
                         available_data: Dict[str, Any]) -> ReportPlan:
    ...
    def _create_fallback_plan(self, report_type: str, framework: Optional[str], 
                             available_data: Dict[str, Any]) -> ReportPlan:
...
class ResearchAgent(LoggingMixin):
    ...
    def __init__(self):
        self.research_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert ESG research agent. Your role is to conduct thorough, iterative research on specific topics using available documents.
...
- "recommendations": Specific recommendations based on findings"""),
    ...
    async def research_topic(self, question: str, framework: Optional[str], 
                           max_iterations: int = 3) -> ResearchFinding:
...
class ValidationAgent(LoggingMixin):
    ...
    def __init__(self):
        self.validation_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert ESG validation agent. Your role is to fact-check findings, ensure accuracy, and identify inconsistencies.

For each finding to validate, you should:
1. Cross-reference information across multiple sources
2. Check for consistency with established ESG standards
...
class SynthesisAgent(LoggingMixin):
    ...
    def __init__(self):
        self.synthesis_prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert ESG report synthesis agent. Your role is to combine research findings into well-structured, professional report sections.
...
Create comprehensive, detailed content that demonstrates deep analysis and understanding of ESG principles."""),
            ("human", """
...
""")
    ...
    async def synthesize_section(self, section: Dict[str, Any], findings: List[ResearchFinding],
...
class AgenticRAGOrchestrator(LoggingMixin):
    ...
    async def generate_report(
        ...
        **kwargs
        ...
        self.logger.info("Starting agentic report generation", 
                        report_type=report_type, framework=framework)
        ...
        try:
            ...
            for question in plan.research_questions:
                ...
                self.logger.info("Research completed", topic=question, confidence=finding.confidence_score)
            ...
            avg_confidence = sum(f.confidence_score for f in research_findings) / len(research_findings) if research_findings else 0
            ...
            confidence_scores = {
                ...
                if i < len(research_findings) else 0.5
            ...
            return AgenticRAGResponse(
                ...
                validation_results={
                    ...
                    "overall_status": "validated" if all(v.get("validation_status") == "validated" for v in validation_results) else "needs_review"
                },
            ...
            raise ESGAnalysisException(f"Agentic RAG failed: {str(e)}")
    
    def _calculate_quality_score(self, validation_results: List[Dict[str, Any]], 
                               research_findings: List[ResearchFinding]) -> float:
        ...
        research_confidence = sum(
            f.confidence_score for f in research_findings
    ...
    async def stream_report_generation(
        ...
        **kwargs
        ...
        try:
            ...
            plan = await self.planner.create_plan(report_type, framework, available_data)
            yield {"status": "progress", "phase": "planning", "message": f"Plan created with {plan.estimated_sections} sections"}
            
            # Phase 2: Research
            research_findings = []
            for i, question in enumerate(plan.research_questions):
                yield {"status": "progress", "phase": "research", 
            ...
            for i, finding in enumerate(research_findings):
                yield {"status": "progress", "phase": "validation",
            ...
            report_sections = []
            for i, section in enumerate(plan.structure):
                yield {"status": "progress", "phase": "synthesis",
